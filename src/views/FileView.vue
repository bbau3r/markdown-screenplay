...
const fileContent = ref<string>(file.content)
const characters = ref<string[]>(file.characters)
const scenes = ref<SceneData[]>(file.scenes)
...
<template>
  <v-sheet class="pa-4 ma-3" elevation="3" v-if="fileContent">
    <div v-html="fileContent" class="sp-container"></div>
  </v-sheet>
    ...
    >
      <div>
        <v-tabs v-model="tabs" color="secondary" grow>
          ...
          <v-tab :value="2" prepend-icon="mdi-format-list-bulleted-square"></v-tab>
        </v-tabs>
      </div>
      <div class="flex-grow-1 overflow-y-auto">
        <v-tabs-window v-model="tabs" class="ma-2">
          <v-tabs-window-item :key="1" :value="1">
            <characters-section :characters="characters"></characters-section>
          </v-tabs-window-item>
          <v-tabs-window-item :key="2" :value="2">
            <scene-section :scenes="scenes"></scene-section>
          </v-tabs-window-item>
        </v-tabs-window>
      </div>
    </div>
  ...
  <v-slide-x-reverse-transition>
    <v-btn
      ...
      elevation="12"
      v-if="!mdAndUp && fileContent && showFab"
